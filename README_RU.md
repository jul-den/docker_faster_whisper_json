...
- **Гибкое описание шагов** – поддерживаются операции:
  - `concat` – склейка видео или аудиофайлов (поддержка масок, автоматическое приведение к единым параметрам для аудио).
...
- **Модели Whisper** могут быть предварительно загружены в кэш (`/app/cache`). При первом запуске без кэша будет попытка скачать модель – это нормально, но для офлайн-режима после первой загрузки модель останется в кэше.
...
#### `concat` – склейка файлов (видео или аудио)

- `input` – **строка или список строк**, определяющих исходные файлы. Каждый элемент может содержать glob-символы (`*`, `?`, `[]`) для поиска в папке `/source`.  
  Примеры: `"media_*.ts"`, `["part1.wav", "part2.wav", "extra_*.wav"]`.
- `output` – имя результирующего файла (будет создан в подпапке задачи).
- `type` – тип склейки: `"video"` или `"audio"`.  
  - Для `video` файлы просто копируются (без перекодирования) с помощью `ffmpeg concat` – подходит для `.ts`, `.mp4` и т.п.  
  - Для `audio` выполняется приведение всех файлов к единым параметрам и их последовательная склейка.
- `params` – объект с параметрами для аудио (игнорируется для видео):
  - `ar` – частота дискретизации (Гц). Если указано `"auto"`, то будет выбрана минимальная частота среди всех склеиваемых файлов. Если не указано – используется `16000`.
  - `ac` – количество каналов. Если `"auto"` – минимальное количество каналов среди файлов. Если не указано – `1`.
...
#### Простая склейка аудиофайлов с автоопределением параметров
```json
{
...
#### Полный пример для вебинара (несколько шагов)
```json
{
  "type": "video_transcription",
  "name": "webinar",
...
- `LLM_URL` – URL эндпоинта LLM (LM Studio, по умолчанию `http://host.docker.internal:1234/v1/chat/completions`). Можно указать эндпоинт любой совместимой LLM (подобные OpenAI).
...
- Для офлайн-режима рекомендуется предварительно загрузить нужную модель Whisper и смонтировать кэш в папку /app/cache. После этого можно установить переменные `HF_HUB_OFFLINE=1` и `TRANSFORMERS_OFFLINE=1` (они уже прописаны в коде).
...
- Если в шаге `transcribe` указана маска, и `combine: true`, все найденные тексты объединяются. Промежуточные тексты каждого сегмента сохраняются в файлы с именем аудио файла с расширением `.txt` (рядом с WAV).
